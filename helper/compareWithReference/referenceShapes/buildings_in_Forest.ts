export const buildings_in_Forest: any = {
  jobSummaryDescriptor: {
    ids: [725],
    stats: {
      incomePerCapita: 26304,
      housingUnitsTotal: 98119,
      population: 225207,
      unitsInStructure1: 65008,
      unitsInStructure2to9: 12607,
      unitsInStructure10to19: 2959,
      unitsInStructure20more: 4783,
      unitsInStructureMobile: 12762,
      totalUnits: 50844,
      averageUnitPrice: 279932,
      medianUnitPrice: 238187,
    },
    objectCount: {
      small: { ship: 3, "small-vehicle": 29 },
      large: { "baseball-diamond": 1 },
    },
    buildingCount: 16,
    averageBuildingAreaCoverage: 0.0006108858338257178,
    buildingHeight: {
      V1: { averageBuildingHeight: 3.1099891607215255 },
      V2: {
        averageBuildingHeight: 2.445823332518731,
        buildingCount: 7,
        building3lessAreaCoverage: 0.0001308323622417717,
        building3to8AreaCoverage: 0.00007913626876531863,
        building8moreAreaCoverage: 0,
      },
    },
    buildingDamage: {
      V2: {
        buildingNoDamageRate: 0,
        buildingMinorDamageRate: 0,
        buildingMajorDamageRate: 0,
        buildingDestroyedRate: 0,
      },
    },
    populationCount: 1429,
    landUse: {
      V2: {
        NATURE: 0.7535014005602241,
        CULTIVATED: 0.23809523809523808,
        RESIDENTIAL: 0.008403361344537815,
      },
    },
    economicExposureValue: { V2: 3504778 },
    insuranceExposureValue: {
      V2: { AGRI: 0, COMMERCIAL: 0, INDUSTRIAL: 0, RESIDENTIAL: 2803822 },
    },
    derivativeFriendlyRate: 1,
    averageBuildingHeight: 3.1099891607215255,
  },
  links: [],
};
